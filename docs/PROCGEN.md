...
const world = new WorldGenerator('amber-lagoon-42');   // 34 islands, 12 gates, wind, clouds
```
...
| `tree.ts`, `rock.ts`, `cloud.ts`, `ruins.ts`, `props.ts` | Trees (pine, broadleaf, palm, dead; canopies flagged `wind`), rocks and glowing crystals, puffy clouds, arches/columns/stone rings/stone lanterns, turbines (hub + separately spinning blades), windsocks, paper lanterns, balloons, motes, wind streaks, feathers. |
| `glider.ts`, `bird.ts`, `ring.ts` | Player glider per livery (forward is -Z, span ≈ 3.4), bird poses for flocks, race rings (torus in XY, fly through along Z). |
...
```ts
const W = new WorldGenerator('amber-lagoon-42', { islands: 34, spacing: 92, chunkSize: 160, gates: 12, clouds: 70 });
W.islands      // IslandSpec[]: position, radius, height, depth, biome, name, chunk
W.path         // the spiral flow path the archipelago follows
W.rings        // RingSpec[]: the race course (index, position, yaw, radius)
...
grouped into contiguous biome regions; gates sit on the path about 100 units
apart with gentle height changes; wind currents connect stretches of the path;
spawns are spread around the start line. `ChunkTracker.update(pos, radius)`
...
`name/<group>` for each group. When the renderer supports vertex colours it
