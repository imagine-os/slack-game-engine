...
export function createSkyState(): SkyState {
  return {
    ...
    zenith: new Color(), horizon: new Color(), ground: new Color(), ambientSky: new Color(), ambientGround: new Color(), fogColor: new Color(), night: 0,
  };
}
...
interface SkyKey { e: number; zenith: string; horizon: string; ground: string; sun: string; sunI: number }

/** Palette keyframes by sun elevation (degrees). */
const KEYS: SkyKey[] = [
  { e: -90, zenith: '#04060f', horizon: '#0a1020', ground: '#05060a', sun: '#000000', sunI: 0 },
  { e: -14, zenith: '#060a18', horizon: '#111a33', ground: '#06070c', sun: '#000000', sunI: 0 },
  { e: -6, zenith: '#131c3d', horizon: '#3d2a55', ground: '#0d0b12', sun: '#4a2a3a', sunI: 0 },
  { e: 0, zenith: '#2f4b82', horizon: '#ff9457', ground: '#2a1d1a', sun: '#ff7132', sunI: 0.35 },
  { e: 6, zenith: '#4a7dc1', horizon: '#ffc78f', ground: '#3d2f26', sun: '#ffb26a', sunI: 0.85 },
  { e: 15, zenith: '#3f7fd6', horizon: '#e6ddc8', ground: '#4a4034', sun: '#ffe6c2', sunI: 1 },
  { e: 45, zenith: '#2f6ccf', horizon: '#cfe0f0', ground: '#4a4a48', sun: '#fff5e6', sunI: 1 },
  { e: 90, zenith: '#2a62c4', horizon: '#c9ddf0', ground: '#4a4a48', sun: '#fff9f0', sunI: 1 },
];
const KEY_COLORS = KEYS.map((k) => ({ zenith: Color.fromHex(k.zenith), horizon: Color.fromHex(k.horizon), ground: Color.fromHex(k.ground), sun: Color.fromHex(k.sun) }));
...
export function evaluateSky(p: SkyParams, out: SkyState): SkyState {
  ...
  for (const c of [out.zenith, out.horizon, out.ground, out.sunColor]) {
    c.r *= p.tint.r * p.exposure; c.g *= p.tint.g * p.exposure; c.b *= p.tint.b * p.exposure; c.a = 1;
  }
  out.night = smooth(clamp((2 - deg) / 10, 0, 1));
  // Fog sits between horizon and zenith.
  out.fogColor.copy(out.horizon).lerp(out.zenith, 0.12);
  // Hemisphere ambient.
  out.ambientSky.copy(out.horizon).lerp(out.zenith, 0.45).scale(0.55 * p.ambientIntensity);
